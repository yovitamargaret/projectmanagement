...
function Project() {
    ...
    let date= (due_date)=>{
        ...
        let calculatedDays = Math.round((end_date - now)/ oneDay);
        let result = "";
        if(calculatedDays < 0) {
            result = Math.abs(calculatedDays) + " days overdue";
        }
        else if(calculatedDays > 0){
            result = calculatedDays + " days remaining";
        }
        else{
            result = "Today is due"
        }
    ...
    return (
        <>
        <p>Username: {UserData().employee_name}</p>
        <p>Role: {UserData().role_name}</p>
        <div className='p-5 bg-light'>
            <div className="container-fluid">
                <div className="col-3 mb-2">
                    <Button onClick={() => setLgShow(true)} hidden={isHidden(2)}>Add Project</Button>
                </div> 
            ...
            <Row>
                ...
                <Row>
                    ...
                    <Col ><ProgressBar now={now} label={`${now}%`} visuallyHidden /></Col>
                     <Col>
                        <div className="time">
                        <Badge bg="secondary">{date(project.due_date)}</Badge>
                        </div>
                    </Col>
                </Row>
                </div>
